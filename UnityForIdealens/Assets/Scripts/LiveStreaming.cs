...
public class LiveStreaming : MonoBehaviour
{
    ...
    void Start () {
        ...
        using (AndroidJavaClass cls = new AndroidJavaClass("com.ricoh.livestreaming.unity.UnityPlugin"))
        {
            // UnityPluginにUnityのMainThread内で作成したeglContextを設定する。
            // WebRTCライブラリで作成したTextureをUnityで利用するために必要な処理。
            // Player Settingsの"Multithreaded Rendering"をOFFに設定しておく必要がある。
            cls.CallStatic("setUnityContext");
        }
